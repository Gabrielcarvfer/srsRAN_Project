...
e2sm_rc_control_action_3_1_du_executor::e2sm_rc_control_action_3_1_du_executor(du_configurator& du_configurator_) :
  e2sm_rc_control_action_du_executor_base(du_configurator_, 1)
{
  // Control Action description:
  // To request the handover of the primary cell and subsequent secondary cell reselection
  action_name = "Handover control request";
  ...
  action_params.insert({16, "List of QoS flow in the DRB"});
                                                                              ...
                                                                              du_mac_sched_control_config& ctrl_cfg)
{
  ...
  //  }
  control_config_params cur_control_params = {};
  if (action_params[ran_param_id] == "Target Primary Cell ID") {
  } else if (action_params[ran_param_id] == "CHOICE Target Cell") {
  } else if (action_params[ran_param_id] == "NR Cell") {
  } else if (action_params[ran_param_id] == "NR CGI") {
    if(ran_param.ran_p_choice_elem_false().ran_param_value_present)
    {
      cur_control_params.mobility_manager.emplace();
      cur_control_params.mobility_manager.value().nr_cgi.emplace();
      auto bytes = (uint64_t)ran_param.ran_p_choice_elem_false().ran_param_value.value_int();
      auto plmn = (uint32_t)(bytes >>36); // upper 28-bit part ideentifies plmn
      auto nci = (bytes << 28) >> 28; // lower 36-bit part identifies nci
      cur_control_params.mobility_manager.value().nr_cgi.value() = nr_cell_global_id_t(plmn, nci);
    }
  } else if (action_params[ran_param_id] == "E-UTRA Cell") {
  } else if (action_params[ran_param_id] == "E-UTRA CGI") {
    if(ran_param.ran_p_choice_elem_false().ran_param_value_present)
    {

    }
//} else if (action_params[ran_param_id] == "List of PDU sessions for handover") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "PDU session Item for handover") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "PDU Session ID") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "List of QoS flows in the PDU session") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "QoS flow Item") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "QoS Flow Identifier") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "List of DRBs for handover") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "DRB item for handover") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "DRB ID") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "List of QoS flow in the DRB") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "QoS flow Item") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "QoS flow Identifier") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "List of Secondary cells to be setup") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "Secondary cell Item to be setup") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
//} else if (action_params[ran_param_id] == "Secondary cell ID") {
//  logger.error("Unsupported RAN parameter ID {}:{}", ran_param_id, action_params[ran_param_id]);
  } else {
    logger.error("Unknown RAN parameter ID {}", ran_param_id);
    return;
  }
  if (cur_control_params.mobility_manager.has_value())
  {
    ctrl_cfg.param_list.push_back(cur_control_params);
  }
}
...
e2sm_ric_control_response e2sm_rc_control_action_3_1_du_executor::convert_to_e2sm_response(
    const du_mac_sched_control_config&          du_config_req_,
    const du_mobility_management_handover_control_config_response& du_response_)
{
  e2sm_ric_control_response e2sm_response;
  e2sm_response.success = du_response_.accepted
                          and du_response_.target_plmnid == du_config_req_.param_list.back().mobility_manager.value().nr_cgi.value().plmn_id.to_bcd()
                          and du_response_.target_nci == du_config_req_.param_list.back().mobility_manager.value().nr_cgi.value().nci.value();
